...
int main()
{
    ...
    tankOneGroup =
            dynamic_cast<osg::Group*>(osgDB::readNodeFile(
                    "/home/lzt/material/gliderZoomIn.osg"));
    //   tankTwoGroup = dynamic_cast<osg::Group*>
    //      (osgDB::readNodeFile("/home/lzt/Downloads/NPS_Tutorials_src/NPS_Data/Models/t72-tank/t72-tank_des.flt"));
    tankThreeGroup =
            dynamic_cast<osg::Group*>(osgDB::readNodeFile(
...
//    tankOneGroup->accept(*(new findNodeVisitor("")));
    ...
    //   if (! (tankOneGroup && tankTwoGroup &&
    //   tankThreeGroup) )
    //   {
    //      std::cout << "could not load tank models" <<
    //      std::endl;
    //      return -1;
    //   }
    ...
    //   // declare a transform for positioning the second
    //   tank
    //   osg::PositionAttitudeTransform* tankTwoPAT =
    //      new osg::PositionAttitudeTransform();
    ////   // move the second tank five units right, five
    /// units forward
    //   tankTwoPAT->setPosition( osg::Vec3(5,5,0) );
    ////   // add the tank as a child of its transform to
    /// the scene
    ...
    tankThreePAT->setPosition(osg::Vec3(5, 0, 0));
    ...
    //       tankThreePAT->setAttitude(
    //       osg::Quat(3.14159/8.0,
    //       osg::Vec3(0,0,1) ));
    //     add the tank as a child of its transform to the
    //     scene
    root->addChild(tankThreePAT);
    tankThreePAT->addChild(tankThreeGroup);
    //    root->addChild(tankThreeGroup);

    //   // Declare an instance of 'findNodeVisitor' class
    //   and set its
    //   // searchForName string equal to "sw1"
    //   findNodeVisitor findNode("sw1");

    //   // Initiate traversal of this findNodeVisitor
    //   instance starting
    //   // from tankTwoGroup, searching all its children.
    //   Build a list
    //   // of nodes whose names matched the searchForName
    //   string above.
    //   tankTwoGroup->accept(findNode);

    //   // Declare a switch type and assign it to the first
    //   node
    //   // in our list of matching nodes.
    //   osgSim::MultiSwitch* tankSwitch = NULL;
    //   tankSwitch = dynamic_cast <osgSim::MultiSwitch*>
    //   (findNode.getFirst());

    // make sure it's a valid handle. If it is, set the
    // first (only)
    // multi-switch:
    //   if (tankSwitch)
    //   {
    //      //tankSwitch->setSingleChildOn(0,false); // good
    //      model
    //      tankSwitch->setSingleChildOn(0,true); // bad
    //      model
    //   }

    // Declare an instance of 'findNodeVisitor'; set the
    // name to search
    // for to "turret"
    findNodeVisitor findTurretNode("UAV");
    // Initiate a traversal starting from the subtree that
    // represents
    ...
    osgSim::DOFTransform* turretDOF =
            dynamic_cast<osgSim::DOFTransform*>(
                    findTurretNode.getFirst());

    // if it's a valid DOF node, set the heading of the
    // turret
    // to 22.5 degrees right relative to the tank's heading.
    if (turretDOF)
    {
        turretDOF->setCurrentHPR(
...
//        //    Initiate a traversal from the subtree that
//        //    represents the third
//        //    tank model we loaded.
//        tankThreeGroup->accept(findGunNode);

//        // Make sure we found a node and it's the correct
////        type
//        osgSim::DOFTransform* gunDOF =
//                dynamic_cast<osgSim::DOFTransform*>(
//                        findGunNode.getFirst());

////        //    If the node we found is a valid  DOF
////        transform,
////        //    set the pitch
////        //    to 22.5 degrees up relative to the turret's
////        pitch.
//        if (gunDOF)
//        {
//            gunDOF->setCurrentHPR(
//                    osg::Vec3(0.0, 3.14159 / 8.0, 0.0));
//        }
